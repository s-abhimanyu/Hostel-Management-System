...
// code for bulk registration
if (isset($_POST['bulk'])) {
    // Process bulk registration file upload
    if (isset($_FILES['bulk_registration_file'])) {
        $file_name = $_FILES['bulk_registration_file']['name'];
        $file_tmp = $_FILES['bulk_registration_file']['tmp_name'];
        $file_type = $_FILES['bulk_registration_file']['type'];

        if ($file_type == "text/csv") {
            $file = fopen($file_tmp, "r");
            while (($data = fgetcsv($file, 1000, ",")) !== FALSE) {
                // Extract data for bulk registration from CSV columns
                $course = $data[0];
                $regno = $data[1];
                $fname = $data[2];
                $mname = $data[3];
                ...
                // Prepare and execute query to insert bulk registration data
                ...
                $stmt = $mysqli->prepare($query);
                $stmt->bind_param('ssssssssssssssssssss', $course, $regno, $fname, $mname, $lname, $gender, $contactno, $emailid, $emcntno, $gurname, $gurrelation, $gurcntno, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode);
                $stmt->execute();
            }
            fclose($file);
            echo "<script>alert('Bulk registration completed successfully.');</script>";
        } else {
...
<body>
	...
	<form method="post" action="" class="form-horizontal" onsubmit="return validateForm()">
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title">Bulk Registration </h2>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="panel panel-primary">
									...
									<div class="form-group">
        <label class="col-sm-2 control-label">Bulk Registration File (CSV):</label>
        <div class="col-sm-8">
            <input type="file" name="bulk_registration_file" class="form-control" >

			<input type="submit" name="bulk" value="Bulk Register" class="btn btn-primary">
...
function isValidIndianNumber(number) {
    ...
    var indianNumber = /^[6-9]\d{9}$/;

    console.log("Input number:", number); // Log the input number for debugging

    var isValid = indianNumber.test(number);
...
function validateForm() {
    ...
    var emergencyContact = document.getElementById("econtact").value;
    var guardianContact = document.getElementById("gcontact").value;

    if (!isValidIndianNumber(emergencyContact)) {
        alert("Please enter a valid emergency contact number .");
        return false;
    }

    if (!isValidIndianNumber(guardianContact)) {
        alert("Please enter a valid guardian contact number .");
        return false;
    }
